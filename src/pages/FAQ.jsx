    ...
    <div className="min-h-screen bg-gradient-to-br from-sky-50 via-white to-sky-50 py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="mb-16">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            {filteredFAQs.map((faq, index) => (
              <div
                key={index}
                className={`bg-white/80 backdrop-blur-sm rounded-2xl shadow-lg border border-blue-100 overflow-hidden transition-all duration-300 hover:shadow-xl ${
                  openIndex === index ? 'ring-2 ring-blue-500' : 'hover:border-blue-300'
                }`}
              >
                <button
                ...
                >
                  <div className="flex items-center space-x-4">
                    <div className="flex-shrink-0 p-3 bg-sky-100 rounded-xl text-sky-600 group-hover:bg-sky-500 group-hover:text-white transition-colors">
                      {React.cloneElement(faq.icon, { className: 'w-5 h-5' })}
                    </div>
                    <h3 className="text-lg font-semibold text-gray-900 group-hover:text-sky-700 transition-colors">{faq.question}</h3>
                  </div>
                  {openIndex === index ? (
                    <ChevronUp className="h-5 w-5 text-sky-600 group-hover:text-sky-700 transition-colors" />
                  ) : (
                ...
                <div className={`px-6 pb-6 ml-16 transition-all duration-300 ${
                  openIndex === index ? 'block' : 'hidden'
                }`}>
                  <div className={`px-6 pb-6 transition-all duration-300 ${openIndex === index ? 'block animate-fade-in' : 'hidden'}`}>
                    <p className="text-gray-700 bg-sky-50/50 p-4 rounded-lg border-l-4 border-sky-300">{faq.answer}</p>
                  </div>
                </div>
              </div>
            ))}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-16">
          <div className="bg-gradient-to-br from-sky-400 to-sky-500 rounded-2xl p-8 text-white shadow-lg hover:shadow-xl transition-all hover:-translate-y-0.5">
            <div className="text-4xl mb-4">💼</div>
            <h3 className="text-xl font-bold mb-3">Contingency-Based</h3>
            <p className="text-sky-100">No recovery = No fees. Period.</p>
          </div>
          
          <div className="bg-gradient-to-br from-sky-500 to-sky-600 rounded-2xl p-8 text-white shadow-lg hover:shadow-xl transition-all hover:-translate-y-0.5">
          ...
          <div className="bg-gradient-to-br from-sky-600 to-sky-700 rounded-2xl p-8 text-white shadow-lg hover:shadow-xl transition-all hover:-translate-y-0.5">
            <div className="text-4xl mb-4">🚀</div>
        ...
        {/* Contact CTA */}
        <div className="mt-16 bg-gradient-to-br from-sky-50 to-white rounded-2xl p-10 shadow-lg border border-sky-100">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
              ...
              <div className="bg-blue-50 p-8 rounded-2xl">
                ...
                <form className="space-y-4">
                  ...
                  <button
                    type="button"
                    className="w-full bg-gradient-to-r from-blue-600 to-blue-700 text-white font-semibold py-3 px-6 rounded-lg hover:from-blue-700 hover:to-blue-800 transition-all duration-300 shadow-md"
                  >
                    Send Message
                  </button>
                </form>
